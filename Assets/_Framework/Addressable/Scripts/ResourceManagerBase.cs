...
namespace Ironcow
{
    public class ResourceManagerBase<T> : MonoSingleton<T> where T : ResourceManagerBase<T>
    {
        ...
        public List<T> LoadAssets<T>(string key) where T : UnityEngine.Object
        {
            List<T> retList = new List<T>();
            var keys = new List<string>(assetPools.Keys);
            foreach (var val in keys)
            {
                if (val.Contains(key.Split('/').Last()))
                {
            ...
            if (retList.Count == 0)
            {
                retList = Resources.LoadAll<T>(key).ToList();
                foreach (var item in retList)
                {
                    if (!assetPools.ContainsKey(item.name))
                        assetPools.Add(item.name, item);
                }
            }
            return retList;
        }
        ...
        {
            return LoadAssets<T>("Datas");
        }
#else
        public async Task<List<T>> LoadDataAssets<T>() where T : BaseData
        {
