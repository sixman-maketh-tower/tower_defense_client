...
#elif USE_COROUTINE
                WorldBase worldObject = null;
                ResourceManager.instance.LoadAsset<WorldBase>(data.rcode, eAddressableType.prefab, obj =>
                {
...
#elif USE_ASYNC                                                                      
                var worldObject = Instantiate(await ResourceManager.instance.LoadAsset<WorldBase>(data.rcode, eAddressableType.prefab), position, Quaternion.identity, ObjectPool);
#else
                var worldObject = Instantiate(ResourceManager.instance.LoadAsset<WorldBase>(data.rcode), position, Quaternion.identity, ObjectPool);
#endif
