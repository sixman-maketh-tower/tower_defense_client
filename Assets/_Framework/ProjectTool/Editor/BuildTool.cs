...
namespace Ironcow
{
    public class BuildTool : EditorWindow
    {
        ...
        PlayerSettings.Android.useAPKExpansionFiles = this.applicationSettings.CurrentBuildSettings.AppBundle;
 ...
 BuildPipeline.BuildPlayer( EditorBuildSettings.scenes, fullPath, BuildTarget.iOS, buildOptions );
        ...
        {
            var fileName = string.Format("DungeonSurvivors{0}.{1}", applicationSettings.CurrentBuildSettings.TargetService == BuildSettings.Service.DEVELOPER ? "_Test" :
                applicationSettings.CurrentBuildSettings.TargetService == BuildSettings.Service.STAGING ? "_Staging" : "",
                applicationSettings.CurrentBuildSettings.TargetService == BuildSettings.Service.APP_BUNDLE ? "aab" : "apk");

            var target = File.ReadAllBytes(path);

            var ftpUrl = new Uri("ftp://newto.synology.me:10021/web/ds_apk/resource/");
            var request = WebRequest.Create(ftpUrl + fileName) as FtpWebRequest;

            request.Credentials = new NetworkCredential("FTP_user", "QWEqwe123");
            ...
            //await SendMessageAsync(fileName + " 업로드 완료 다운로드 링크 : http://newto.synology.me/ds_apk/");
            this.actionUpload = false;
        }
